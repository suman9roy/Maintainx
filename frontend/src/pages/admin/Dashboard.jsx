...
export default function AdminDashboard() {
  ...
  const formatDate = (value) => {
    ...
    if (Number.isNaN(date.getTime())) return 'Pending payment';
    ...
    <Layout>
      ...
      <section style={s.section}>
        <div style={s.sectionHeader}>
          ...
          <span style={s.helperText}>{transactions.length} records</span>
        </div>

        <div style={s.transactionList}>
          {visibleTransactions.length === 0 ? (
            <div style={s.empty}>No transaction history yet.</div>
          ) : visibleTransactions.map(tx => (
            <div key={tx.id} style={s.transactionCard}>
              <div>
                <div style={s.activityTitle}>Flat {tx.flatNumber} • {tx.month} {tx.year}</div>
                <div style={s.activityMeta}>{formatDate(tx.paidAt ?? tx.updatedAt ?? tx.createdAt)}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ ...s.amountText, color: tx.paymentStatus === 'PAID' ? '#059669' : '#d97706' }}>
                  ₹{Number(tx.amount).toLocaleString('en-IN')}
                </div>
                <span style={{ ...s.badge, background: STATUS_COLOR[tx.paymentStatus]?.bg, color: STATUS_COLOR[tx.paymentStatus]?.fg }}>
                  {tx.paymentStatus}
                </span>
              </div>
            </div>
...
const s = {
  ...
  transactionList: { maxHeight: 320, overflowY: 'auto', paddingRight: 4 },
  transactionCard: { background: '#fff', borderRadius: 8, padding: '12px 14px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' },
  amountText:   { fontSize: 14, fontWeight: 700, marginBottom: 6 },
  paginationRow:{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 },
  pageBtn:      { border: '1px solid #cbd5e1', background: '#fff', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', fontSize: 13 },
